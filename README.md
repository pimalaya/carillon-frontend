...
Run `npm run dev` for a hot-reloading dev server, `npm run build` for the production bundle, and `npm run test` for the unit tests. Every screen maps onto the carillon-backend control API described by the OpenAPI contract at [carillon-backend/docs/openapi.yaml](../backend/docs/openapi.yaml); the architecture, screen map and build status live in the [docs](./docs) folder.
