...
const meta: Record<StreamStatus, { label: string; dot: string; text: string }> = {
  ...
  connecting: { label: 'Connecting', dot: 'bg-warning', text: 'text-muted-foreground' },
  live: { label: 'Live', dot: 'bg-success', text: 'text-foreground' },
  stale: { label: 'Reconnecting', dot: 'bg-warning', text: 'text-muted-foreground' },
};
...
export function LiveIndicator({ status }: { status: StreamStatus }) {
  ...
  const animate = status === 'live' || status === 'connecting' || status === 'stale';
  return (
