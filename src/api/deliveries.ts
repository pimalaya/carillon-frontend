...
function prepend(
  ...
  qc.setQueryData<Delivery[]>(key, (old) =>
    old ? [d, ...old].slice(0, CAP) : old,
  );
}
...
export function useLiveDeliveries(
  enabled = true,
): ReturnType<typeof useEventStream> {
  const { activeLink } = useAuth();
  ...
  return useEventStream(enabled, (event) => {
    if (event.type === "delivery") {
      ...
      patchMeWatch(qc, activeLink, d.account, (w) => ({
        ...w,
        lastEventAt: d.at,
      }));
      ...
      // Billing notice — surface it and refresh the credit/activation state.
      const label =
        event.kind === "watch_ending"
          ? "A watch is about to end"
          : event.kind === "low_pool"
            ? "Credits running low"
            : "A watch stopped — activate it to resume";
